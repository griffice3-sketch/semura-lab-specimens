...
| [013](./specimens/013-msnr-curve-fitting-defects/) | マレーシアSNR・多層コンフルエンス型（マジックナンバーの全身転移と多重保有による過剰最適スクリプト） | HTFのSNRと多層シグナルを束ねるコンフルエンス設計の裏で、過去データを丸暗記したマジックナンバーの全身転移による重度な過剰最適化と、単一ポジション原則を破る多重保有リスクが露呈した致命的病巣を解剖。 | https://semura-lab.com/report/013 https://medium.com/@griffice3/specimen-013-malaysian-snr-multi-layer-confluence-model-an-over-optimized-script-plagued-by-22c6acefe751 |
| [014](./specimens/014-ema-rsi-reverse-reference-bug/) | EMA・RSIトレンド順張り型（時系列の逆参照による致命的誤作動と検証データ皆無の空虚スクリプト） | 損小利大とリスク基準ロットを採用した設計思想は健全だが、配列の時系列を逆参照する致命的バグにより実質的に逆張りで誤作動する病巣と、エビデンス数値が一切存在しない検証欠落の三重苦を解剖。 | https://semura-lab.com/report/014 https://medium.com/@griffice3/specimen-014-ema-rsi-trend-following-type-fatal-malfunction-via-time-series-reverse-reference-f11d69b86b32 |
| [015](./specimens/015-mtf-rsi-fake-d1-bug/) | マルチタイムフレームRSIモメンタム順張り型（日足参照欠落による設計上の欺瞞と前提条件の崩壊） | 上位足（日足）のモメンタムに追従するという基本設計を謳いながら、実際のコードではD1データの参照が完全に欠落している設計上の欺瞞を解剖。マルチタイムフレーム分析の看板に偽りがあり、単一時間軸のRSIのみで無防備なエントリーを繰り返すハリボテの病巣を摘出する。 | https://semura-lab.com/report/015 https://medium.com/@griffice3/specimen-015-multi-timeframe-rsi-momentum-follower-design-fraud-via-missing-d1-reference-and-da25e2c3580f |
| [016](./specimens/016-mtf-rsi-bug/) | 一目均衡表ベース・常時ドテン型（実体なき幻のMM計算による自己欺瞞と損切り完全欠如が招く口座破壊装置） | バックテストデータが不在の中、一目均衡表の遅行スパンと雲のクロスを根拠とする常時ドテン型の構造を解剖。ストップロスの完全欠如という致命傷に加え、計算上のみ存在する「幻の2%リスク」によるMMの自己欺瞞と、スプレッド・時間フィルタの不在により、市場の摩擦と急変で確実に口座を破壊する致命的病巣を摘出する。 | https://semura-lab.com/report/016 https://medium.com/@griffice3/specimen-016-ichimoku-based-continuous-stop-and-reverse-an-account-destroyer-driven-by-b4a7efda0b62 |
| [017](./specimens/017-stoch-loop-bug/) | Stochastic閾値クロス・単一ポジション型（MAを騙る実装矛盾による自己認識の欠落と、摩擦防御の完全不在が露呈する検証不能なコピペ残骸） | バックテストデータが不在の中、MAのロジックを謳いながら実体はStochasticの単純な逆張りという実装の矛盾を解剖。スプレッドや時間フィルタなど摩擦への防御機構が完全に欠落しており、未確定足（バー0）参照や決済ループのバグといった致命的欠陥を抱える、検証不能なコピペテンプレートの病巣を摘出する。 | https://semura-lab.com/report/017 https://medium.com/@griffice3/specimen-017-stochastic-threshold-cross-single-position-type-a659407feb53 |
